...
test_capr %>% select('location-lat', 'location-long', utm.easting, utm.northing) %>% 
  summarise_all(mean) # só vamos usar das duas primerias colunas
...
#Planilha com dados ambientais (variáveis explicativas) - arquivo CaprinosRSFannotate_variaveis.csv com as variáveis 
...
#' Center and scale variables (faz sentido para só uma variável numérica?)
...
#' precisamos saber da proporção entre zeros e uns para atribuir peso
str(rsfdat_capr)
rsfdat_capr$case_fac<-as.factor(rsfdat_capr$case_)
summary(rsfdat_capr$case_fac) # agora seiu a proporção de zeros e uns
